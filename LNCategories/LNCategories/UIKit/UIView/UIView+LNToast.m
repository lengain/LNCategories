...
- (void)makeToastAnimationImageWithSize:(CGSize)size withImageArray:(NSArray *)imageArray isdismiss:(BOOL)isdismiss{
    ...
    UIView *activityView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, CSToastActivityWidth - 20, CSToastActivityHeight - 20)];
    activityView.center = [self centerPointForPosition:CSToastActivityDefaultPosition withToast:activityView];
    activityView.backgroundColor = [UIColor colorWithWhite:1 alpha:1];
    ...
    if (imageArray) {
        ...
        NSString *categoryBuondlePath = [[NSBundle mainBundle] pathForResource:@"LNCategories-resources" ofType:@"bundle"];
        NSBundle *categoryBuondle = [NSBundle bundleWithPath:categoryBuondlePath];
        NSString *indicatorGIFPath = [categoryBuondle pathForResource:@"app_indicator" ofType:@"gif"];
        NSData *indicatorGIFData = [[NSData alloc] initWithContentsOfFile:indicatorGIFPath];
        UIImage *indicator = [UIView ln_animatedGIFWithData:indicatorGIFData];
...
//copy sd_animatedGIFWithData
+ (UIImage *)ln_animatedGIFWithData:(NSData *)data {
    if (!data) {
        return nil;
    }
    ...
    if (count <= 1) {
        ...
        for (size_t i = 0; i < count; i++) {
            ...
            duration += [self ln_frameDurationAtIndex:i source:source];
...
+ (float)ln_frameDurationAtIndex:(NSUInteger)index source:(CGImageSourceRef)source {
